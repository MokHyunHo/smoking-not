...
public class TokenRefreshDialog extends Dialog {
		...
		mRefreshButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if (!Utility.mFacebook.extendAccessToken(activity, listener)) {
					listener.onError(new Error(activity
							.getString(R.string.refresh_token_binding_error)));
				}
			}
		});
	}
	...
	private class RefreshTokenListener implements Facebook.ServiceListener {
		...
		public void onFacebookError(FacebookError e) {
			...
			String title = String.format(
					activity.getString(R.string.facebook_error) + "%d",
					e.getErrorCode());
		...
		public void onError(Error e) {
			changeButtonState(true);
			Util.showAlert(activity, activity.getString(R.string.error),
					e.getMessage());
		}
	...
	private void changeButtonState(boolean enabled) {
		...
		mRefreshButton.setText(enabled ? R.string.refresh_button
				: R.string.refresh_button_pending);
	}
